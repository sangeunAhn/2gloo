...
import {StyleSheet, Dimensions, View, TouchableOpacity, Platform, SafeAreaView, Text, Linking} from 'react-native';
...
export default class Main extends Component {
	...
	constructor(props) {
		...
		this.state = {
			kindsOrder: [],
			collapseArray: [true, true, true, true, true, true, true, true, true, true],
			existClub: '생성',
		};
	...
	render() {
		const {navigation} = this.props;
		const {kindsOrder, collapseArray, existKindsOrder, notExistKindsOrder} = this.state;
		const t = this;
						...
						{kindsOrder.map((kinds, i) => {
							return (
								<Collapse
									...
									onToggle={isCollapsed => this.setState(collapseArray.splice(i, 1, isCollapsed))}>
									<CollapseHeader>
									...
									<CollapseBody>
										<ClubDiv key={i} clubKind={kinds} school={schoolName} {...this.props} />
									</CollapseBody>
								</Collapse>
							);
						})}
